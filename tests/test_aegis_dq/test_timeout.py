"""Unit tests for AegisDQRunTimeout and max_wait_seconds parameter.

Tests cover:
- AegisDQRunTimeout exception class attributes and message
- wait_for_run() raises TimeoutError when deadline exceeded
- run_checks() catches TimeoutError and re-raises as AegisDQRunTimeout
...
# ---------------------------------------------------------------------------
# AegisDQRunTimeout exception class
# ---------------------------------------------------------------------------

def test_run_timeout_is_exception_subclass():
    exc = AegisDQRunTimeout(profile="dev", run_id=42, max_wait_seconds=60)
...
def test_wait_for_run_raises_timeout_error_when_deadline_exceeded(monkeypatch):
    ...
    client = AegisAPIClient(api_url="http://fake-aegis", api_key="test-key")
...
def test_wait_for_run_uses_monotonic_clock():
    ...
    import aegis_dq._client as client_module
    import inspect
    source = inspect.getsource(client_module.AegisAPIClient.wait_for_run)
...
def test_run_checks_passes_max_wait_seconds_to_wait_for_run(monkeypatch):
    ...
    with patch("aegis_dq._run.AegisAPIClient", return_value=mock_client):
...
def test_run_checks_raises_aegis_timeout_when_wait_times_out(monkeypatch):
    """run_checks() must convert TimeoutError from wait_for_run into AegisDQRunTimeout."""
    monkeypatch.setenv("AEGIS_API_URL", "http://fake-aegis")
    monkeypatch.setenv("AEGIS_API_KEY", "test-key")
